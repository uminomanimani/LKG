...
if __name__ == "__main__":
    ...
    teacher_data = json.load(open(f"./saves/{teacher_model}/{dataset}/teacher_output/entity_description_{dataset_type}.json"))
    ...
    os.makedirs(f"./saves/{teacher_model}/{dataset}/fine_tuning_for_description_extraction", exist_ok=True)    
    with open(f"./saves/{teacher_model}/{dataset}/fine_tuning_for_description_extraction/ner_sft_{dataset_type}.json", "w", encoding="utf-8") as f:
        json.dump(out, f, indent=4, ensure_ascii=False)
    
    dataset_info = {}

    dataset_info[f"{dataset_type}"] = {"file_name" : f"ner_sft_{dataset_type}.json"}

    # 如果已经存在则覆盖掉
    with open(f"./saves/{teacher_model}/{dataset}/fine_tuning_for_description_extraction/dataset_info.json", "w", encoding="utf-8") as f:
